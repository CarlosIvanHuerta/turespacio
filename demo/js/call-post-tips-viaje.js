...
const getPostCategoryTipsTravel = () => {
   const params = new URLSearchParams({
      stage: 'categoryTipsTravel',
      tax_slut: CATEGORY_SLUT_POST.POST_TRAVEL_TIPS,
      tax_name: CATEGORY_NAME_POST.POST_TRAVEL_TIPS,
      tax_id: CATEGORY_TERM_ID_POST.POST_TRAVEL_TIPS,
      limit: 6
   });
   ...
   return fetch(`conx/functions-data.php?${params.toString()}`)
      ...
      .then(data => {
         if (data.success && data.counter !== 0) {
            renderPostCategoryTipsTravel(data)
         } else {
...
const renderPostCategoryTipsTravel = (data) => {
   try {

      const container = document.querySelector('#container-post-cat-tips')
      //container.innerHTML = ''
